...
The left sidebar contains the Benchmark Input panel, divided into several distinct sections:

### Auxiliary Tools

At the top of the input panel is a **"Tools"** accordion. Expanding this section exposes auxiliary capabilities to assist in analyzing relations between objects before incorporating them into your benchmark:

- **Get Distance:** When you need spatial context, input the Component IDs of any two objects into the respective fields. Click **Calculate** to instantly query the server and retrieve the exact Euclidean distance (in meters) between the centers of those two components.
...
Write the ideal reasoning and answer to the question. Because the system strictly identifies objects, you must appropriately tag the item instances using their _Component ID_ directly in your response.
