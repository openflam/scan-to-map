#!/usr/bin/env python3
"""Run prepared missing-component conditions without touching legacy benchmark paths."""
...
def preflight_output_dirs(
    ...
    if existing:
        ...
        raise FileExistsError(
            "Refusing to mix a new run with existing condition outputs. "
            "Use --resume to keep existing files and fill only missing ones:\n"
            f"{preview}"
        )
...
def load_database_tables(
    ...
    if existing and not reuse_existing:
        ...
        raise RuntimeError(
            ...
            "loader because it replaces tables: "
            f"{preview}. Use --reuse-existing-db-tables to trust and reuse them."
        )

    to_create = [
        dataset_name for dataset_name in dataset_names if dataset_name not in existing
    ]
    if to_create:
        run_command(
            [
...
def build_parser(repo_root: Path) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run an isolated missing-component sensitivity experiment."
    )
    parser.add_argument(
        "--experiment-dir",
        type=Path,
        default=repo_root / "benchmark" / "missing_components",
        help="Directory created by prepare.py.",
    )
    parser.add_argument(
        "--stage",
        ...
        default="commands",
        help=(
            "Stage to run. The default only prints commands and does not modify "
            "the database or call the search server."
        ),
    )
    parser.add_argument(
        "--conditions",
        nargs="+",
        help="Optional condition IDs to run. Defaults to all conditions.",
    )
    parser.add_argument(
        "--configs",
        nargs="+",
        default=[DEFAULT_CONFIG],
        help="Tool configurations passed to gen_answers.py.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Trust perturbed PostGIS tables that already exist. Without this flag "
            "the loader refuses to call the table-replacing ingestion script."
        ),
    )
    return parser
...
def main() -> None:
    ...
    for condition in conditions:
        questions_dir = resolve_experiment_path(
            experiment_dir, condition["questions_dir"]
        )
        results_dir = resolve_experiment_path(
            experiment_dir, condition["results_dir"]
        )
        metrics_dir = resolve_experiment_path(
            experiment_dir, condition["metrics_dir"]
        )
